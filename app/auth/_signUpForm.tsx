...
const eightYearsAgo = getLuxonDate(new Date(), { utc: false })
	.minus({ years: 8 })
	.toJSDate()
		...
		<YStack width={'100%'} gap={3} alignSelf="center">
			<XStack gap={3} alignItems="center" marginBottom={4}>
				<Text style={{ fontSize: 13 }}>
					Te estas vinculando con tu cuenta de
				</Text>
				{providerController?.renderIcon &&
					providerController.renderIcon()}
			</XStack>
			<OurInput
				label={{ text: 'Nombres' }}
			...
			<OurInput
				...
				onChangeText={(apellidos) =>
					handleChange('lastnames', apellidos)
				}
				formControlProps={{
			...
			<OurDatePicker
				...
				onDateChange={(fechaNacimiento) =>
					handleChange('birthdate', fechaNacimiento)
				}
				formControlProps={{
			...
			<XStack gap={5}>
				<OurInput
					...
					onChangeText={(nroDocumento) =>
						handleChange('document_number', nroDocumento)
					}
					formControlProps={{
				...
				<OurInput
					...
					onChangeText={(telefono) =>
						handleChange('cellphone', telefono)
					}
					formControlProps={{
			...
			<OurButton onPress={handleSubmit} style={{ marginTop: 10 }}>
				Registrarme
			</OurButton>
		</YStack>
	)
